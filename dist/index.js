...
let currentLinkStatuses = [];
exports.currentLinkStatuses = currentLinkStatuses;
let lastUpdate = new Date(0); // Initialize to epoch time
exports.lastUpdate = lastUpdate;
let currentFriendLinksData = null; // Store the original friend links data
/**
 ...
 */
// @ts-ignore
async function fetchFriendLinks(url) {
    try {
 ...
 */
// @ts-ignore
async function checkLinkStatus(url, name) {
 ...
 * @returns Promise<LinkStatus[]>
 */
async function checkAllLinkStatuses(friendLinksData) {
    ...
    const allLinks = friendLinksData.friends
        ...
        .flatMap(category => category.link_list.map(link => ({ name: link.name, link: link.link })));
...
async function runDailyCheck() {
    try {
        ...
        const linksToCheck = friendLinksData.friends
            ...
            .flatMap(category => category.link_list.map(link => ({ ...link })));
